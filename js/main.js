    ...
    success: async function (res) {
      ...
      await res.map((icon) => {
        ...
        categories.forEach((category) => {
          ...
          if (
            (category == "fab" && icon[1] == 1) ||
            (category != "fab" && icon[1] != 1)
          ) {
            var btnElem = $("<button/>", { class: "copy" }).html(
      ...
      myTable.ready(function () {
        ...
        console.log(
          "%c Script Writter : Rakibul Islam",
          "color: #0f84f1; font-size:14px; padding: 12px"
        );
        console.log(
          "%c website Designer: Tahir Ahmad",
          "color: #0f84f1; font-size:14px; padding: 12px"
        );
        console.log(
          "%c Develped by Lazy Developers Community (LDC)",
          "color: #0f84f1; font-size:14px; padding: 12px"
        );
